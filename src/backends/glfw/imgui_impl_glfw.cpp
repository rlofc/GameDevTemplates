...
// In this binding, ImTextureID is used to store an OpenGL 'GLuint' texture identifier. Read the
// FAQ about ImTextureID in imgui.cpp.

// If your context is GL3/GL3 then prefer using the code in opengl3_example.
// You *might* use this code with a GL3/GL4 context but make sure you disable the programmable
...
// setting up 'RenderDrawListsFn' in the ImGuiIO structure)
...
bool ImGui_ImplGlfw_Init(GLFWwindow* window, bool install_callbacks)
{
    ...
    ImGuiIO& io = ImGui::GetIO();
    io.KeyMap[ImGuiKey_Tab] = GLFW_KEY_TAB;  // Keyboard mapping. ImGui will use those indices
    ...
    io.RenderDrawListsFn =
        ImGui_ImplGlfw_RenderDrawLists;  // Alternatively you can set this to NULL and call
                                         // ImGui::GetDrawData() after ImGui::Render() to get
...
{
    if (!g_FontTexture) ImGui_ImplGlfw_CreateDeviceObjects();
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED)) {
        ...
        io.MousePos = ImVec2((float)mouse_x, (float)mouse_y);  // Mouse position in screen
                                                               // coordinates (set to -1,-1 if
                                                               // no mouse / on another screen,
                                                               // etc.)
    }
    else {
